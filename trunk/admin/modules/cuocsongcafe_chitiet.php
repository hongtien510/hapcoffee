	...
	$r=mysql_fetch_assoc($rs);
		...
		if(isset($_POST['submit']))
		{
			...
			if($TieuDeBGT==""){
				...
				if($r['tenbaiviet']==""){
					$sql2="insert into baiviet values(NULL,'$TieuDeBGT','$NoiDungBGT','','$AnHien','cuocsoncafe')";
					mysql_query($sql2);
				...
				if($r['tenbaiviet']!=""){
					$sql2="update baiviet set tenbaiviet='$TieuDeBGT', 
											 ...
											 alias='', 
											 anhien='$AnHien', 
											 loaibaiviet='cuocsoncafe' 
											 where mabv=".$r['mabv'];
					//echo $sql2;			 
					mysql_query($sql2);
					echo '<script>alert("Cập nhật thành công");window.location="?action=CuocSongCafe";</script>';
				}
			}
		}
?>
